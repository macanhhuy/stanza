...
function Client(opts) {
    ...
    this.use(require('./plugins/httpupload'));
    ...
    this.on('stream:data', function (data) {
        ...
        if (data._name === 'iq') {
            ...
            if (data.xml && data.xml.children && data.xml.children[0] && data.xml.children[0].nodeName === 'slot') {
                self.emit('vnc:httpupload', data.xml);
            }
    ...
    this.on('iq', function (iq) {
        ...
        if (iq.type === 'get' || iq.type === 'set') {
            ...
            console.log('xxxxx', iqEvent, self.callbacks);
